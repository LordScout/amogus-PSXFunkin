...
#define CAR_START_Y FIXED_DEC(-800,1)
#define CAR_END_Y    FIXED_DEC(500,1)
#define CAR_TIME_A FIXED_DEC(1,1)
#define CAR_TIME_B FIXED_DEC(2,1)
...
void Back_Week4_DrawMD(StageBack *back)
{
	...
	if (this->car_timer <= 0)
	{
		...
		this->car_y = CAR_START_Y;
	}
	
	if (this->car_y < CAR_END_Y)
		this->car_y += timer_dt * 3000;
	
	//Draw car
	fx = stage.camera.x * 4 / 3;
	fy = stage.camera.y * 4 / 3;
	
	RECT car_src = {0, 0, 70, 246};
	RECT_FIXED car_dst = {
		FIXED_DEC(-160,1) - fx,
		this->car_y - fy,
		FIXED_DEC(300,1),
...
void Back_Week4_DrawBG(StageBack *back)
{
	...
	RECT_FIXED sunset_dst = {
		FIXED_DEC(-270 - SCREEN_WIDEOADD2,1) - fx,
		FIXED_DEC(-240,1) - fy,
		FIXED_DEC(620 + SCREEN_WIDEOADD,1),
		FIXED_DEC(480,1)
	};
